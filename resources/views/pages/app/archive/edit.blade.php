...
@section('content')
  <form method="POST" action="{{ route('admin.archive.update', $archive->id) }}" x-data="{ processing: false }"
    @submit="processing = true" class="max-w-md mx-auto space-y-4">
    @csrf
    <h1 class="font-semibold text-xl">Edit Quarterly Archive</h1>
    <div class="flex w-full flex-col gap-1 text-neutral-600">
      <label for="volume" class="w-fit pl-0.5 text-sm">Volume</label>
      <input id="volume" type="text" value="{{ $archive->volume }}"
    ...
    <div class="flex w-full flex-col gap-1 text-neutral-600">
      ...
      <input id="issue" type="text" value="{{ $archive->issue }}"
        class="w-full rounded-sm border border-neutral-300 bg-neutral-50 px-2 py-2 text-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black disabled:cursor-not-allowed disabled:opacity-75"
        name="issue" required />
      @error('issue')
        <small class="pl-0.5 text-red-500">{{ $message }}</small>
    ...
    <div class="grid grid-cols-2 gap-4">
      <div class="flex w-full flex-col gap-1 text-neutral-600">
        ...
        <input id="from_month" type="month" value="{{ $archive->from_month }}"
          class="w-full rounded-sm border border-neutral-300 bg-neutral-50 px-2 py-2 text-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black disabled:cursor-not-allowed disabled:opacity-75"
      ...
      <div class="flex w-full flex-col gap-1 text-neutral-600">
        ...
        <input id="to_month" type="month" value="{{ $archive->to_month }}"
          class="w-full rounded-sm border border-neutral-300 bg-neutral-50 px-2 py-2 text-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black disabled:cursor-not-allowed disabled:opacity-75"
